...
function [score, result] = single_graph_test(size, weight_range,connected_rate,noise_rate,atr_flag)
...
M = triu(randn(size)*weight_range,1);    %  upper left part of a random matrix with weight_range
...
test_nodes_atrs=NaN;
if atr_flag
    nodes_atrs=randn([1,size])*weight_range;
...
disp(['Recall: ' num2str(score(1)/score(2))])
disp(['Precision: ' num2str(score(1)/score(3))])
disp(['Pattern Size: ' num2str(score(2))])
